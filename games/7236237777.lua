...
Notify("Anti Auto Clicker has been disabled.")
end)

plr:AddCheckBox("Gas Range+", function() use:Disconnect() Notify("Infinite Gas Range disabled.") end, function()
    use = game:GetService("RunService").Stepped:Connect(function()
		...
		Notify("UnnamedHub", "Infinite Gas Range enabled.")
end)

MoneyMod = plr:AddNumberBox("Get Money", function()
    local Money = MoneyMod:GetNumber() or 1
	if not LP.PlayerGui:FindFirstChild("GasUI") then Notify("You need to interact with a gas station first !") return end
...
if not game:GetService("Workspace").Vehicles:FindFirstChild(game:GetService("Players").LocalPlayer.Name.."'s Car") then Notify("Unnamed Hub", "No owned car found.") return end


if not game:GetService("Workspace").Vehicles[game:GetService("Players").LocalPlayer.Name.."'s Car"].Body["Vehicle Damage Model"].Main:FindFirstChild("TouchInterest") then Notify("Unnamed Hub", "Car is already invisible") return end
...
CarMod = car:AddNumberBox("Mod Speed", function()
    ...
    Notify("UnnamedHub", "Changed Car Speed.")
end)
...
car:AddButton("Fix Car", function()
	local car = GetCar()
	if car == nil then Notify("You must input a valid player name.") return end
	Notify("UnnamedHub", "Vehicule Fixed.")
...
car:AddButton("Horn", function()
	local car = GetCar()
	if car == nil then Notify("You must input a valid player name.") return end
	if not car:FindFirstChild("Horn") then Notify("The user's car isn't supported yet.") return end
